"""
API-key authentication for the Domain 8 gateway.

Disabled by default (local development / CI). Enable in config.json:
  "api": { "auth": { "enabled": true, "api_key": "...", "api_key_env": "VEXER_ORI_API_KEY" } }
...
def configured_api_key() -> Optional[str]:
    ...
    if not auth.get("enabled", False):
        return None
    return os.getenv(auth.get("api_key_env") or "VEXER_ORI_API_KEY") or auth.get("api_key")
